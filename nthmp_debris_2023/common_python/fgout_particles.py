    ...
    for ns in range(nsubsteps):
        ...
        for dbno in dbnos:
            ...
            try:
                ...
                hfacedb = hface  # assume it's a scalar, same for all debris
                
            try:
                wfacedb = wface[dbno]
            except:
                wfacedb = wface  # assume it's a scalar, same for all debris
            ...
            v1 = v_fcn(xd1,yd1,t1)
            if isnan(u1):
                print('+++ dbno=%i, xd1=%.3f, evaluate u1=%.3f' % (dbno,xd1,u1))
            
            if df is None:
                # set debris velocity equal to fluid velocity (tracer particle)
                ud2 = u1
                vd2 = v1
                #print('+++ dbno = %4i, Tracer, u1 = %.2f, v1 = %.2f' % (dbno,u1,v1))
            else:
                ...
                if wfacedb > 0:
                    # hydrodynamic force 
                    #  = (density of water) * (area of impact) * (velocity diff)^2
...
def make_debris_paths_substeps(fgout_grid, fgframes, debris_paths, dbnos,
                      drag_factor=None, grounding_depth=0.,
                      mass=1e9, wface=0., hface=0., dradius=None, 
                      Kspring=None, tether=None, nsubsteps=1):
    """
    dbnos is a list of debris particle labels (integers) to operate on.
    debris_paths a dictionary indexed by integer dbno.
