...
subroutine bc2amr(val,aux,nrow,ncol,meqn,naux, hx, hy, level, time,   &
    ...
    if (xlo_patch < xlower-hxmarg) then
        ...
        select case(mthbc(1))
            case(-1) ! User defined boundary condition
                
                !s = 0.72d0 * exp(-0.4*(time - 28.35d0)**2)   ! matches specified wavemaker
                
                s = 0.25d0 * exp(-0.04*(time - 20.d0)**2)
                do j = 1,ncol
                 do i=1,nxl
                    do m=1,meqn
            ...
            case(0) ! Wall until t=300, then outflow
               if (time < 300.d0) then
                hmax = val(1,1,1)
                do j = 1, ncol
                    do i=1, nxl
                ...
                do j = 1, ncol
                    ...
                    val(1,1:nxl,j) = 0.877d0 + &
                            (hmax - 0.877d0) * exp(-(time-300.d0)/50.d0)
                end do
               endif
