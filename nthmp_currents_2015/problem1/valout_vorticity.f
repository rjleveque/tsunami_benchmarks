...
c     -------------------------------------------------
c
c     Version that also outputs vorticity, so fort.q files have 5 columns:
c         h, hu, hv, eta, vorticity
c
c
      use amr_module
...
c     # set outaux = .true. to also output the aux arrays to fort.a<iframe>
      ...
      real(kind=8), allocatable :: qetav(:)
      real(kind=8), allocatable :: u(:,:), v(:,:), vorticity(:,:)
     ...
     .                                      naux*mitot*(j-1)
      iaddqetav(ivar,i,j)  = 1 + ivar - 1 + 5*((j-1)*mitot+i-1)
c
...
c                 # output in 1d format if ny=1:
 ...
 1004 format(e18.8,'    xlow', /,
     &       e18.8,'    dx', /)
        ...
        allocate(u(mitot,mjtot), v(mitot,mjtot))
        allocate(vorticity(mitot,mjtot))

        do j = nghost, mjtot-nghost+1
            do i = nghost, mitot-nghost+1
        ...
        do j = nghost+1, mjtot-nghost
           do i = nghost+1, mitot-nghost
                ! Calculate vorticity
                uy = (u(i,j+1) - u(i,j-1)) / (2.d0*hyposs(level))
                vx = (v(i+1,j) - v(i-1,j)) / (2.d0*hxposs(level))
                vorticity(i,j) = vx - uy
                if (abs(vorticity(i,j)) < 1d-90) then
                    vorticity(i,j) = 0.d0
                    endif
                enddo
            enddo
        ...
        if (output_format == 1) then
             do j = nghost+1, mjtot-nghost
                do i = nghost+1, mitot-nghost
                   ...
                   write(matunit1,109) h,hu,hv,eta,vorticity(i,j)
                enddo
                write(matunit1,*) ' '
             enddo
  109        format(50e26.16)
...
c            # Need to augment q with eta:
             allocate(qetav(5*mitot*mjtot))
             do j=1,mjtot
                 do i=1,mitot
                    do m=1,3
                        qetav(iaddqetav(m,i,j)) = alloc(iadd(m,i,j))
                    enddo
                    eta = alloc(iadd(1,i,j)) + alloc(iaddaux(1,i,j))
                    qetav(iaddqetav(4,i,j)) = eta
                    qetav(iaddqetav(5,i,j)) = vorticity(i,j)
                 enddo
             enddo
...
c     # Print meqn = nvar+2 because eta and vorticity added.
