...
c     write(6,*) '+++ in advanc'
      do  j = 1, numgrids(level)
          ...
          xhi = rnode(cornxlo,mptr) + nx*hx
          yhi = rnode(cornylo,mptr) + ny*hy
          if ((yhi > 19.757) .and. (204.93 >= xlow+nghost*hx) 
     &        .and. (204.93<=xhi)) then
             ii = (204.93 - xlow) / hx
             jj = ny+nghost
c            write(6,*) '+++ setting eta: ii,jj,mitot,mjtot'
c            write(6,*) ii,jj,mitot,mjtot
              do i=1,780
                 if (incident_t(i) > time) go to 444
                 enddo
 444          continue
              tau = (time - incident_t(i-1)) /
     &              (incident_t(i) - incident_t(i-1))
              if ((tau<0) .or. (tau>1)) then
                 write(6,*) '*** interpolation error, time,tau,i: '
                 write(6,*) time,tau,i
                 stop
                 endif
             eta_star = (1.d0-tau)*incident_eta(i-1) 
     &                  + tau*incident_eta(i)
             h_int = alloc(iadd(1,ii,jj)) ! interior value of depth
             h_star =  eta_star - alloc(iaddaux(1,ii,jj))  ! desired value
             amu_int = alloc(iadd(3,ii,jj))  ! interior value of momentum
             amu_star = amu_int - (h_star - h_int) * 
     &            dsqrt(grav * 0.5d0 * (h_int + h_star))
             dh_star = h_star - h_int
             dmu_star = amu_star - amu_int
c            write(6,*) '  eta_star, B: ',eta_star, 
c    &                     alloc(iaddaux(1,ii,jj))
c            write(6,*) '  h_int, amu_int: ',h_int, amu_int
c            write(6,*) '  h_star, amu_star: ',h_star, amu_star
c            write(6,*) '  dh_star, dmu_star: ',dh_star, dmu_star
             endif
          enddo
c ===============================
c
